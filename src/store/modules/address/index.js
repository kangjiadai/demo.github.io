import {getAddressData,delAddressData,addAddressData,getAddressInfoData,modAddressData,getDefaultAddreaData} from '../../../api/address';
    ...
    actions:{
        ...
        delAddress(conText,payload){
            delAddressData({uid:conText.rootState.user.uid,...payload}).then(res=>{
                if(res.code===200){
                    if(payload.index){
                        conText.commit("DEL_ADDRESS",{index:payload.load})
                    }
                }
            })
        },
        ...
        getDefaultAddress(conText,payload){
            getDefaultAddreaData(conText.rootState.user.uid).then(res=>{
                if(payload.success){
                    payload.success(res);
                }
            })
