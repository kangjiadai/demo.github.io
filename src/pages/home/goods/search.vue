                ...
                <div class='attr-wrap'>
                    <div class='attr-title-wrap' @click="HIDE_PRICE()">
                        ...
                        <div class='price-wrap' @click.stop>
                            <div class='price-input'><input type="tel" placeholder="最低价" @keyup="SET_MINPRICE({minPrice:$event.target.value})" :value="minPrice"/></div>
                            <div class='price-line'></div>
                            <div class='price-input'><input type="tel" placeholder="最高价" :value="maxPrice" @keyup="SET_MAXPRICE({maxPrice:$event.target.value})"/></div>
                        </div>
...
<script>
    ...
    export default {
        ...
        methods:{
            ...mapMutations({//调用vuex中的 HIDE_PRICE 方法
            ...
            selectPriceOrder(index){//价格排序
                if(this.priceOrderList.length>0){
                    for(let i = 0;i < this.priceOrderList.length;i++){
                        if( this.priceOrderList[i].active){
                            this.priceOrderList[i].active=false; //当选中其中一个时，其他元素清除样式
                            break;
                        }
                    }
                    this.priceOrderList[index].active=true;
                    this.$set(this.priceOrderList,index,this.priceOrderList[index]); //防止视图不渲染问题
                    this.isSalesOrder = false;//点击综合时清除销量样式
            ...
            selectSales(){//销量排序
                ...
                for(let i = 0;i < this.priceOrderList.length;i++){//点击销量时清除综合排序样式
                    if( this.priceOrderList[i].active){
                        this.priceOrderList[i].active=false; //当选中其中一个时，其他元素清除样式
                        break;
                    }
        ...
        beforeRouteUpdate(to,from,next){
            ...
            this.isPriceOrder = false; //第二次搜索清除之前综合样式
            this.otype="all"; //第二次搜索清除之前价格排序
            this.resetScreen();
            this.isSalesOrder = true; //第二次搜索清除销量排序
            if(this.priceOrderList.length>0){//第二次搜索清除之前颜色
                for(let i = 0;i < this.priceOrderList.length;i++){
                    if( this.priceOrderList[i].active){
                        this.priceOrderList[i].active=false; //当选中其中一个时，其他元素清除样式
                        break;
            ...
            this.priceOrderList[0].active=true;//默认为综合选项
            this.init();
            this.getAttrs({keyword:this.keyword,success:()=>{
                    this.myScroll.refresh();
                }});
