<template>
    <div class="page">
        ...
        <div class='bottom-btn-wrap'>
            ...
            <div class='btn cart' @click="showPanle()">加入购物车</div>
        </div>
        ...
        <div ref="cart-panel" :class="isPanel?'cart-panel up':'cart-panel down'">
            ...
            <div class='attr-wrap'>
                <div class='attr-list' v-for="(item,index) in attrs" :key="index">
                    ...
                    <div class='val-wrap'>
                        <span :class="{'val':true, 'active':item2.active}" v-for="(item2,index2) in item.values" @click="SELECT_ATTR({index:index,index2:index2})">{{item2.value}}</span>
                    </div>
                </div>
...
<script>
    ...
    export default {
        ...
        methods:{
            ...
            //添加购物车半场动画
            addCart(){
                if(this.isMove){ //判断执行动画时，不能关闭面板
                    this.isMove=false;
                    let goodsImg = this.$refs['goods-img'],goodsInfo = this.$refs["goods-info"];
                    let cloneImg = goodsImg.cloneNode(true); //克隆一张图片，用以做添加动画
                    goodsInfo.appendChild(cloneImg);
                    let cartTop = window.innerHeight-this.$refs['cart-panel'].offsetHeight; //计算动画最终执行完成高度
                    ...
                    TweenMax.to(cloneImg, 2, {bezier:[{x:cloneImg.offsetLeft, y:-100},{x:cartIcon.offsetLeft, y:-cartTop}],onComplete:()=>{
                            cloneImg.remove();
                            this.isMove=true;//动画执行完毕后可以关闭面板
                            ...
                            if(this.attrs.length>0){
                                for(let i = 0;i < this.attrs.length;i++){
                                    param=[]; //当第一层循环结束时清空，防止第二层循环数据重复添加
                                    for(let j = 0;j < this.attrs[i].values.length;j++){
                                        if(this.attrs[i].values[j].active){ //当前已经被选中
                                            param.push({paramid:this.attrs[i].values[j].vid,title:this.attrs[i].values[j].value})
                                        }
                                    }
                                    attrs.push({attris:this.attrs[i].attrid,title:this.attrs[i].title,param:param})
                                }
                            }
...
<style scoped>
    ...
    .cart-panel .amount-wrap .amount-input-wrap .amount-input input{width:100%;height:95%;text-align: center;border:none}
