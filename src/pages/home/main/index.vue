...
<script>
    export default {
        ...
        methods:{
            ...
            changeStyleNav(name){
                switch(name){//判断当前页面，并给底部栏导航添加样式
                    case "index"://商城
                        this.homeStyle=true;
        ...
        activated(){//解决keepAlive缓存问题,此时created中的执行过程必不可少，必须执行created后才会产生缓存
            document.title=this.$route.meta.title;
            this.changeStyleNav(this.$route.name);
        }
    }
...
<style scoped>
    .bottom-nav{width: 100%;height: 1.2rem;background-color: white;box-shadow: 0 0 10px #efefef;
    ...
    .bottom-nav ul{width: 1rem;position:relative}
    ...
    .bottom-nav ul .spot{width: 0.2rem;height: 0.2rem;position: absolute;right: 0;top:0;z-index: 2;background-color:#eb1625;border-radius: 0.2rem }
    /*首页*/
